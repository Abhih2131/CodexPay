...
describe('pay insights engine', () => {
  ...
  test('tax slabs, rebate, surcharge, marginal relief, cess', () => {
    ...
    const near50L = calculateTax(5133269);
    expect(near50L.surcharge).toBeGreaterThan(0);
    expect(near50L.marginalRelief).toBeGreaterThan(0);
  ...
  test('marginal relief uses correct active threshold band', () => {
    const above1Cr = calculateTax(10005000);
    const above50L = calculateTax(5005000);
    expect(above1Cr.marginalRelief).toBeGreaterThanOrEqual(0);
    expect(above1Cr.totalAnnualTax).toBeGreaterThan(above50L.totalAnnualTax);
  });
