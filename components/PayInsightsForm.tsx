...
const groupClass = 'rounded-2xl border border-slate-700/40 bg-slate-950/25 p-4';
const labelClass = 'text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-300';
const inputClass = 'mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white/95 px-3 text-slate-900 shadow-sm transition focus:border-sky-400 focus:ring-2 focus:ring-sky-200';
...
export function PayInsightsForm({ onSubmit }: { onSubmit: (input: SimulationInput) => void }) {
  ...
  return (
    <section className={`${cardClass} space-y-5`}>
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">Compensation Input Setup</h2>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className={`${groupClass} grid gap-4 md:grid-cols-2`}>
          ...
          <label className="block md:col-span-2"><span className={labelClass}>City</span><select className={inputClass} value={form.city} onChange={(e) => { update('city', e.target.value); update('hraPercentage', deriveDefaultHra(e.target.value)); }}>{cities.map((s) => <option key={s}>{s}</option>)}</select><p className="mt-1 text-xs text-slate-300">Default HRA for city: {helper}% (editable below)</p></label>
        </div>
